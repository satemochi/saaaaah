...
class ga_tw:
    def __init__(self, g, max_generation=20, population_size=100,
                 max_tournament_entries=20, crossover_rate=0.9,
                 mutation_rate=0.01, ):
        self.g = g
    ...
    def draw_history(self):
        data = defaultdict(lambda: [0] * len(self.history))
        _min, _max = 100, 0
        ...
        w = _max - _min
        my_color = {k: cm.rainbow(float(k - _min) / w) for k in list(df)}
        df.plot(kind='bar', legend=False, stacked=True, align='center',
                width=1.0, color=map(my_color.get, df.columns))
        plt.tight_layout()
        plt.savefig('fitness_distributions_01.png', bbox_inches='tight')
        plt.show()
