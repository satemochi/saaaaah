...
def display():
    ...
    weights = np.random.uniform(0., 0.1, n)    # sampling from [0, 0.1)
...
def draw_cone(x, y, w):
    ...
    for xi, yi in zip(base_x, base_y):
        glVertex3f(xi, yi, -w + radius)     # base circle
    glEnd()
    ...
    # draw circle as additively weight
    glColor4f(1., 1., 1., 0.9)
    glBegin(GL_LINE_LOOP)
    base_x, base_y = x + w * np.cos(t), y + w * np.sin(t)
    for xi, yi in zip(base_x, base_y):
        glVertex3f(xi, yi, -1)
    glEnd()
...
if __name__ == '__main__':
    ...
    glutCreateWindow("additively weighted Voronoi drawing")
