...
def rational_search(a, p, q, d):
    ...
    higher, lower = binary_search(a, p, q, (1 << (k-1)) - 1, (1 << k) - 1, d)
    if d == direction.LEFT:
        new_p = mpq(q.numerator + lower * p.numerator,
                    q.denominator + lower * p.denominator)
        new_q = mpq(q.numerator + (lower - 1) * p.numerator,
                    q.denominator + (lower - 1) * p.denominator)
        if new_q == a:
            return new_q
        return rational_search(a, new_p, new_q, direction.RIGHT)
    else:
        new_q = mpq(p.numerator + lower * q.numerator,
                    p.denominator + lower * q.denominator)
        new_p = mpq(p.numerator + (lower - 1) * q.numerator,
                    p.denominator + (lower - 1) * q.denominator)
        if new_p == a:
            return new_p
...
class test_rational_search(unittest.TestCase):
    ...
    def test_binary_search_left(self):
        ...
        _min, _max = binary_search(a, p, q, (1 << (k-1)) - 1, (1 << k) - 1,
                                   direction.LEFT)
        low = mpq(q.numerator + _max * p.numerator,
                  q.denominator + _max * p.denominator)
        high = mpq(q.numerator + _min * p.numerator,
                   q.denominator + _min * p.denominator)
    ...
    def test_binary_search_right(self):
        ...
        higher, lower = binary_search(a, p, q, (1 << (k-1)) - 1, (1 << k) - 1,
                                      direction.RIGHT)
        high = mpq(p.numerator + higher * q.numerator,
                   p.denominator + higher * q.denominator)
        low = mpq(p.numerator + lower * q.numerator,
                  p.denominator + lower * q.denominator)
